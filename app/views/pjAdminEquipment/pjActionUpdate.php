...
if (isset($tpl['status']))
{
	...
	?>
	<?php pjUtil::printNotice(__('infoUpdateEquipmentItemTitle', true, false), __('infoUpdateEquipmentItemDesc', true, false)); ?>
	
	<?php if ((int) $tpl['option_arr']['o_multi_lang'] === 1 && count($tpl['lp_arr']) > 1) : ?>
	<div class="multilang"></div>
	<?php endif; ?>
	
	<form action="<?php echo $_SERVER['PHP_SELF']; ?>?controller=pjAdminEquipment&amp;action=pjActionUpdate" method="post" id="frmUpdateEquipment" class="form pj-form" autocomplete="off" enctype="multipart/form-data">
		<input type="hidden" name="equipment_update" value="1" />
		...
		<p>
			...
			<span class="inline_block t5">
				<input type="checkbox" id="multi_units" name="multi_units" value="T"<?php echo $tpl['arr']['multi_units'] == 'T' ? ' checked="checked"' : NULL;?>/>
			</span>
		</p>
		<p>
			...
			<span class="inline_block">
				<span class="pj-form-field-custom pj-form-field-custom-before float_left r10">
					...
					<input type="text" id="price" name="price" value="<?php echo $tpl['arr']['price'];?>" class="pj-form-field number w108 required" data-msg-number="<?php __('pj_number_validation');?>"/>
				</span>
				<select name="price_per" id="price_per" class="pj-form-field">
					<option value="booking"<?php echo $tpl['arr']['price_per'] == 'booking' ? ' selected="selected"' : NULL;?>><?php __('lblPricePerBooking');?></option>
					<option value="hour"<?php echo $tpl['arr']['price_per'] == 'hour' ? ' selected="selected"' : NULL;?>><?php __('lblPricePerHour');?></option>
				</select>
			</span>
		</p>
		<p>
