		...
		if (is_array($tpl['arr']))
		{
			...
			if ($count > 0)
			{
				...
				<div class="clear_both">
					...
					for ($i = 0; $i < $count; $i++)
					{
						if ($tpl['arr'][$i]['tab_id'] != 8 || (int) $tpl['arr'][$i]['is_visible'] === 0) continue;
						...
						<tr class="pj-table-row-odd<?php echo $rowClass; ?>" style="<?php echo $rowStyle; ?>">
							<td width="35%" valign="top">
								...
								<?php if (!empty($opt_text)) { ?>
									<span class="fs10"><?php echo nl2br($opt_text); ?></span>
								<?php } ?>
							</td>
							<td>
								<?php
								switch ($tpl['arr'][$i]['type'])
								{
									...
									case 'text':
											...
											foreach ($tpl['lp_arr'] as $v)
											{
												...
												<p class="pj-multilang-wrap" data-index="<?php echo $v['id']; ?>" style="display: <?php echo (int) $v['is_default'] === 0 ? 'none' : NULL; ?>">
													<span class="inline_block">
														<textarea name="i18n[<?php echo $v['id']; ?>][<?php echo $tpl['arr'][$i]['key'] ?>]" class="pj-form-field" style="width: 400px; height: 400px;"><?php echo pjSanitize::html(@$tpl['arr']['i18n'][$v['id']][$tpl['arr'][$i]['key']]); ?></textarea>
														<?php if ((int) $tpl['option_arr']['o_multi_lang'] === 1 && count($tpl['lp_arr']) > 1) : ?>
														<span class="pj-multilang-input"><img src="<?php echo PJ_INSTALL_URL . PJ_FRAMEWORK_LIBS_PATH . 'pj/img/flags/' . $v['file']; ?>" alt="" /></span>
														<?php endif; ?>
													</span>
