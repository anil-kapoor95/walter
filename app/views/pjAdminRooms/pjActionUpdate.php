...
if (isset($tpl['status']))
{
	...
	?>
	<?php pjUtil::printNotice(__('infoUpdateRoomTitle', true, false), __('infoUpdateRoomDesc', true, false)); ?>
	
	<?php if ((int) $tpl['option_arr']['o_multi_lang'] === 1 && count($tpl['lp_arr']) > 1) : ?>
	<div class="multilang"></div>
	<?php endif; ?>
	
	<form action="<?php echo $_SERVER['PHP_SELF']; ?>?controller=pjAdminRooms&amp;action=pjActionUpdate" method="post" id="frmUpdateRoom" class="form pj-form" autocomplete="off" enctype="multipart/form-data">
		...
		<?php
		if(!empty($tpl['arr']['image_thumb']))
		{
			$image_url = PJ_INSTALL_URL . $tpl['arr']['image_thumb'];
			?>
			<p id="image_container">
				<label class="title">&nbsp;</label>
				<span class="inline_block">
					<img class="mrbs-image" src="<?php echo $image_url; ?>" />
					<a href="javascript:void(0);" class="pj-delete-image" data-href="<?php echo $_SERVER['PHP_SELF']; ?>?controller=pjAdminRooms&amp;action=pjActionDeleteImage&id=<?php echo $tpl['arr']['id'];?>"><?php __('btnDelete');?></a>
				</span>
			</p>
			<?php
		} 
		...
		<p>
			...
			<span class="inline_block">
				<input type="text" name="capacity" id="capacity" value="<?php echo $tpl['arr']['capacity'];?>" class="pj-form-field field-int required digits w80" data-msg-digits="<?php __('pj_digits_validation');?>"/>
		...
		<p id="multidayPrice" class="priceTitle none">
			...
			<span class="pj-form-field-custom pj-form-field-custom-before">
				...
				<input type="text" id="price_per_day" name="price_per_day" value="<?php echo $tpl['arr']['price_per_day'];?>" class="pj-form-field number w108" data-msg-number="<?php __('pj_number_validation');?>"/>
			</span>
		</p>
		<p id="halfdayPrice" class="priceTitle none">
			...
			<span class="pj-form-field-custom pj-form-field-custom-before">
				<span class="pj-form-field-before"><abbr class="pj-form-field-icon-text"><?php echo pjUtil::formatCurrencySign(NULL, $tpl['option_arr']['o_currency'], ""); ?></abbr></span>
				<input type="text" id="price_half_day" name="price_half_day" value="<?php echo $tpl['arr']['price_half_day'];?>" class="pj-form-field number w108" data-msg-number="<?php __('pj_number_validation');?>"/>
			</span>
		</p>
			...
			<span class="inline_block">
				...
				{ 
					...
					<select name="layout_id[]" id="layout_id" multiple="multiple" size="5" class="pj-form-field w300">
						...
						{
							?><option value="<?php echo $v['id']; ?>"<?php echo in_array($v['id'], $tpl['layout_id_arr']) ? ' selected="selected"' : NULL;?>><?php echo stripslashes($v['title']); ?></option><?php
						}
						?>
					</select>
					<?php
